<?php
// Automatic database setup script for admin
...
try {
    ...
    // Step 4: Check and create admin table
    $response['steps'][] = "Checking admin table...";
    $result = $conn->query("SHOW TABLES LIKE 'admin'");
    
    if ($result->num_rows == 0) {
        $response['steps'][] = "Creating admin table...";
        ...
        if ($conn->query($sql) === TRUE) {
            $response['steps'][] = "✓ Admin table created successfully";
            ...
            if ($stmt->execute()) {
                $response['steps'][] = "✓ Default admin user created (AdminID: admin123, Password: 123456)";
            } else {
                $response['steps'][] = "⚠ Warning: Could not create admin user";
            }
            $stmt->close();
        } else {
        ...
        $response['steps'][] = "✓ Admin table already exists";
    }
    
    // Step 5: Check and create users table
    ...
    // Step 6: Check and create assets table
    ...
    if ($result->num_rows == 0) {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            $assetsData = [
                ...
                ['A004', 'Router', 'Networking', 'Cisco Router', 'SN-004', 'IT Department', 'Alice Brown', '2024-02-05', '2027-02-05', 450.00, 'Active', 'Cisco Malaysia'],
                ['A005', 'Chair', 'Furniture', 'Office Chair', 'SN-005', 'HR Department', 'Mike Johnson', '2024-02-10', '2027-02-10', 150.00, 'Active', 'IKEA Malaysia']
            ];
    ...
    $response['steps'][] = "🎉 Setup completed! You can now login to the admin system.";
